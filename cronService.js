        ...
        for (let i = 0; i < rItems.length; i ++) {
            ...
            if (results.length > 0) {
                if (results[0].title != title) {
                   bot.sendMessage(groupId, '🔥Congratulations fellow African <a href="tg://user?id=' + rItem.telegramId+ '">' + rItem.username + '</a>! 🦇 You have just been promoted to **' + title +'**!');
                }
        ...
        checkMission: async function(bot) {
            while(true) {
                ...
                if (missionIds.length > 0) {
                    bot.sendMessage(groupId, '🔥A new mission is available! Go to <a href="https://myafrica.link/tasks">myafrica.link</a> to check the details!');
